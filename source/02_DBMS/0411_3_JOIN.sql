...
-- ★ <연습문제> PART1
--1. 이름, 직속상사명
SELECT E1.ENAME, E2.ENAME "직속상사" 
    FROM EMP E1, EMP E2
    WHERE  E1.MGR = E2.EMPNO;
--2. 이름, 급여, 업무, 직속상사명
SELECT E1.ENAME, E1.SAL, E1.JOB, E2.ENAME "직속상사"
    FROM EMP E1, EMP E2
    WHERE E1.MGR = E2.EMPNO;
--3. 이름, 급여, 업무, 직속상사명 . (상사가 없는 직원까지 전체 직원 다 출력.
    --상사가 없을 시 '없음'으로 출력)
SELECT E1.ENAME, E1.SAL, E1.JOB, NVL(E2.ENAME, '없음') "직속상사" --nvl은 형식 맞춰줘야함
    FROM EMP E1, EMP E2
    WHERE E1.MGR = E2.EMPNO(+);
--4. 이름, 급여, 부서명, 직속상사명
