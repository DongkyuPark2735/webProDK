...
SELECT * FROM TAB;
--2. TO_DATE 단일행함수를 이용하여 EMP 테이블에서 HIREDATE가 1981년 2월 20일부터 1981년 5월 1일 사이에 입사한 사원의 사원명, 직책, 입사일을
--최초입사일부터 최근입사일 순으로 정렬하여 출력하는 SQL문을 작성하시오(5점).
SELECT ENAME, JOB, TO_DATE(HIREDATE, 'YY/MM/DD' ) "입사일"
    FROM EMP
...
SELECT W.ENAME, W.SAL, W.DEPTNO, DNAME, LOC, NVL(M.ENAME, '없음') "상사"
    FROM EMP W, EMP M, DEPT D
    WHERE W.MGR= M.EMPNO(+) AND W.DEPTNO = D.DEPTNO;
--4. SALGRADE 테이블의 LOSAL을 등급최저 급여, HISAL을 등급 최고 급여로 이용하여, EMP 테이블의 사원을 사번, 사원명, 직책, 급여, 급여등급을
