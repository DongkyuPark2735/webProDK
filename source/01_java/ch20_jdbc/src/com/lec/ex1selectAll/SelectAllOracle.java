...
public class SelectAllOracle {
	public static void main(String[] args) {
		...
		// select 문은 결과가 여러줄
		// insert update delete문은 결과가 정수

		try {
			Class.forName(driver); // 1.드라이버 로드
			conn = DriverManager.getConnection(url, "scott", "tiger"); // 2.DB 연결
			stmt = conn.createStatement(); // 3.SQL전송 객체 생성
			rs = stmt.executeQuery(query);// 4 SQL전송 + 5.SQL전송 결과 받기
			// 6. 결과받아서 원하는 로직 수행
			System.out.println("사번\t이름\t직책\t\t상사사번\t입사일\t\t급여\t상여\t부서번호");
			while (rs.next()) {
				int empno = rs.getInt("empno"); // 문자는 숫자로 받아도됨
				// String empno = rs.getString("empno"); // 타이틀 (별칭)을 넣음
