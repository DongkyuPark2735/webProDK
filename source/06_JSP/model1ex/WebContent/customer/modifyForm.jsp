<%@page import="com.lec.dao.CustomerDAO"%>
<%@page import="com.lec.dto.CustomerDTO"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%String conPath = request.getContextPath();%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
<script>
	function infoConfirm(){
		if(frmJoin.cpw.value != frmJoin.cpwChk.value){
			alert('변경하는 비밀번호를 확인하세요');
			frmJoin.cpw.value = '';
			frmJoin.cpwChk.value = '';
			frmJoin.cpw.focus();
			return false;
		}
	}
</script>
<link href="<%=conPath%>/css/join.css" rel="stylesheet">
<%
	CustomerDTO dtos = (CustomerDTO)session.getAttribute("customer");
	CustomerDAO dao = CustomerDAO.getInstance();
	if(dtos==null){
		out.println("<script>alert('로그인상태가 아닙니다.');");
		out.println("location.href='../customer/loginForm.jsp';</script>");
	}
%>
</head>
<body>
	<jsp:include page="../main/header.jsp"/>
	<div id="joinForm_wrap">
		<form action="modifyPro.jsp" method="post" name="frmJoin" onsubmit="return infoConfirm()">
			<div id="join_title">회원가입</div><br>
			<input type="hidden" name="hiddenParam" value="xx">
			<table>
				<tr>
					<th><label for="cid">아이디</label></th>
					<td><input type="text" name="cid" id="cid" required="required"
										 value="<%=dtos.getCid()%>"></td>
				</tr>
				<tr>
					<th><label for="oldpw">기존비밀번호</label></th>
					<td><input type="password" name="oldpw" id="oldpw" required="required"></td>
				</tr>
				<tr>
					<th><label for="cpw">변경 비밀번호</label></th>
					<td><input type="password" name="cpw" id="cpw" placeholder="변경시 입력하세요"></td>
				</tr>
				<tr>
					<th><label for="cpwChk">변경 비밀번호확인</label></th>
					<td><input type="password" name="cpwChk" id="cpwChk" ></td>
				</tr>
				<tr>
					<th><label for="cname">이름</label></th>
					<td><input type="text" name="cname" id="cname" required="required"
										 value="<%=dtos.getCname() %>"></td>
				</tr>
				<tr>
					<th><label for="ctel">전화</label></th>
					<td>
						<input type="tel" name="ctel" id="ctel" required="required" 
									 value="<%=dtos.getCtel() %>">
					</td>
				</tr>
				<tr>
					<th><label for="cemail">이메일</label></th>
					<td><input type="email" name="cemail" id="cemail" 
										 value="<%=(dtos.getCemail()!=null)?dtos.getCemail():"-"%>"></td>
				</tr>
				<tr>
					<th><label for="caddress">주소</label></th>
					<td><input type="text" name="caddress" id="caddress"
										 value="<%=(dtos.getCaddress()!=null)?dtos.getCaddress():"-"%>"></td>
				</tr>
				<tr>
					<th><label for="cbirth">생년월일</label></th>
					<td><input type="date" name="tempbirth" id="tempbirth" required="required"
										 value="<%=dtos.getCbirth() %>"></td>
				</tr>
				<tr>
					<th>성별</th>
					<td>
					<%if(dtos.getCgender().equals("m")){ %>
						<input type="radio" name="cgender" value="m" checked="checked" id="m"><label for="m">남자</label>
						<input type="radio" name="cgender" value="f" id="f"><label for="f">여자</label>
					<%}else if(dtos.getCgender().equals("f")) {%>
						<input type="radio" name="cgender" value="m" id="m"><label for="m">남자</label>
						<input type="radio" name="cgender" value="f" checked="checked" id="f"><label for="f">여자</label>
					<%} %>
					</td>
				</tr>
				<tr>
					<td colspan="2"> </td>
				</tr>
				<tr>
					<td colspan="2">
						<input type="submit" value="수정하기" class="joinBtn_style">
						<input type="reset" value="다시하기" class="joinBtn_style">
						<input type="button" value="로그인" class="joinBtn_style" 
											onclick="location.href='loginForm.jsp'">
					</td>
				</tr>
			</table>
		</form>
	</div>
	<jsp:include page="../main/footer.jsp"/>
</body>
</html>