...
// 6. 글 수정 : public int updateBoard(BoardDTO dto)
	public int updateBoard(BoardDTO dto) { // 글 제목, 글 내용, 비밀번호, 메일
		int result = FAIL;
		//
		Connection conn = null;
		PreparedStatement pstmt = null;
...
// 7. 글 삭제 : public int deleteBorad(int num, String pw)
	public int deleteBorad(int num, String pw) {
		int result = FAIL;
		//
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	//8. 답변글 저장전 step a : UPDATE BOARD SET RE_STEP=RE_STEP+1 WHERE REF= 150 AND RE_STEP > 1;
	...
	public int reply(BoardDTO dto) {
		...
		try {
			...
			System.out.println(result == SUCCESS ? dto+" 성공" : dto+" 실패");
		} catch (SQLException e) {
