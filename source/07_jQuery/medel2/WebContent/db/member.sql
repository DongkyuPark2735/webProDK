...
SELECT * FROM MVC2MEMBER WHERE mID='s' and mPW='1';
-- (2) mid로 dto가져오기(로그인 성공시 session에 넣기 위함)
SELECT * FROM MVC2MEMBER WHERE MID='s';
-- (3) 회원id 중복체크
SELECT * FROM MVC2MEMBER WHERE MID='s';
-- (4) 회원가입
INSERT INTO MVC2MEMBER (MID, MPW, MNAME, MEMAIL, MPHOTO, MBIRTH, MADDRESS)
...
-- (5) 회원정보 수정
UPDATE MVC2MEMBER SET MPW = '1',
                    MNAME = 'HONG',
                    MEMAIL = 'yi@naver.com',
                    MPHOTO = 'NOIMG.JPG',
                    MBIRTH = TO_DATE('1991-11-04', 'YYYY/MM/DD'),
                    MADDRESS = '서울'
        WHERE mId='aaa';
select * from MVC2MEMBER ;
-- (6) 회원리스트(top-N구문)
...
-- (7) 회원수
SELECT COUNT(*) CNT FROM MVC2MEMBER;
COMMIT;
-- (8) 회원탈퇴
DELETE FROM MVC2MEMBER WHERE MID = 'bbb';
ROLLBACK;
-- (9) 이메일 중복체크
SELECT * FROM MVC2MEMBER WHERE mEMAIL = 'go@naver.com';

select * from mvc2member order by mrdate desc;
----------------------------------------------------------------------
--                           FILEBOARD                              --
----------------------------------------------------------------------
...
-- (5) bId로 글 dto보기
SELECT F.*, MNAME FROM FILEBOARD F, MVC2MEMBER M WHERE M.MID=F.MID AND FID=1;
